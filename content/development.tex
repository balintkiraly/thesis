...
Az egyszerűség és teljesség kedvéért a példában (\refstruc{lst:model}) a \lstinline|MyModel| felépítésén mutatom be egy modell definiálását.
...
Az alábbi példában (\refstruc{lst:resolver}) egy egyszerű létrehozásra láthatunk egy lehetséges implementációt.
...
Ezzel valósítottam meg a bejelentkezett felhasználó validálását (\refstruc{lst:shield}).
...
A GraphQL Mutation–t (\refstruc{lst:login}) paraméterekkel ellátva szükséges megírnunk, hogy a kód generátor tudja a lehetséges paramétereket. 
...
A generálást futtatva azonnal használhatjuk az elkészült hook–okat, a csatolt példában (\refstruc{lst:loginForm}) ez a hook a useLoginMutation.
...
\begin{lstlisting}[style=ES6, caption={Bejelentkezés kódrészlet},label={lst:loginForm}]
...
Amennyiben szeretnék eltérni az alapértelmezett beállításoktól, a hibaüzeneteket szövegezését a séma definiálással együtt adhatjuk meg (\refstruc{lst:yup}).
